PHP_ARG_ENABLE(group, whether to enable group, [ --enable-group   Enable Group])

if test "$PHP_GROUP" = "yes"; then

	

	if ! test "x" = "x"; then
		PHP_EVAL_LIBLINE(, GROUP_SHARED_LIBADD)
	fi
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				[ext/pcre/php_pcre.h],
				[
					PHP_ADD_EXTENSION_DEP([group], [pcre])
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				[ext/json/php_json.h],
				[
					PHP_ADD_EXTENSION_DEP([group], [json])
	...
	PHP_INSTALL_HEADERS([ext/group], [php_GROUP.h])
